...
module.exports = {
  scripts: {
     ...
     */
    start: {
      script: 'cross-env NODE_ENV=production node -r dotenv/config dist/index.js',
    ...
    */
    ...
    db: {
      ...
      indexes: {
        script: series(
          'nps banner.indexes',
          runFast('./commands/mongosetup.ts')
        ),
        description: 'Indexes will be added if they do not exist'
      }
    },
    /**
    ...
    test: {
      ...
      unit: {
        ...
        watch: {
          script: 'cross-env NODE_ENV=test jest --watch --testPathPattern=unit',
          hiddenFromHelp: true
        },
        verbose: {
...
function eslint(path) {
  return `eslint -c ./.eslintrc.js --ext .ts '${path}' --format stylish`;
}
